#!/usr/bin/env python3
"""
将目录打包为.docx、.pptx或.xlsx文件的工具，会移除XML格式。
...
def main():
    ...
    try:
        ...
        # 如果验证失败，错误退出
            ...
            print("内容会生成损坏的文件。", file=sys.stderr)
            print("请在重新打包前验证XML。", file=sys.stderr)
            print("使用 --force 跳过验证并继续打包。", file=sys.stderr)
            sys.exit(1)
...
def pack_document(input_dir, output_file, validate=False):
    ...
    参数：
        ...
        validate: 如果为True，使用soffice进行验证(默认: False)

    返回：
        bool: 如果成功返回True，如果验证失败返回False
    """
